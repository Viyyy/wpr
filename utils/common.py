...
def timestr2timedelta(
    datetime_str, format: Union[str, TimeStr] = "%Y-%m-%d %H:%M:%S"
):  # 解析字符串为 datetime 对象
    if isinstance(format, TimeStr):
        format = format.value
...
def round_half_even(value: float, num: int = 1) -> float:
    ...
    """
    return float(
        Decimal(value).quantize(Decimal(f"{10**(-num)}"), rounding=ROUND_HALF_EVEN)
    )
